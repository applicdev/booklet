...
Promise.resolve().then(async function (): Promise<void> {
  ...
  const path = await findPath({ root: !flag['p'] });

  // ?
  if ('v' in flag) {
    await whenOutputVersion().then(() => Deno.exit);
    return;
  }

  // ?
  if ('h' in flag) {
    await whenOutputOptions().then(() => Deno.exit);
    return;
  }
...
async function findPath({ root }: { root: boolean }): Promise<string> {
  ...
  return !root && repo && !repo.name.endsWith('github.io.git') //
